...
interface BranchTarget {
  branch: any;
  target: { disbursementTarget: number; loanCountTarget: number; month: string };
...
export function BranchTargetView() {
  ...
  const [form, setForm] = useState({ disbursementTarget: 0, loanCountTarget: 0, month: '' });
  ...
  const loadBranches = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setBranches(list);
        // Auto-select: BM auto-selects their own branch; others default to first
    ...
    <div className="p-4 lg:p-6 bg-slate-50 min-h-full space-y-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-slate-500">
              {canSetAnyBranch
                ? 'Set monthly disbursement and loan-count targets for any branch. The BM then distributes targets to individual LOs.'
                : isBM
                ? 'Set the monthly target for your branch, then assign individual targets to your Loan Officers via Staff → Staff Detail.'
      ...
      {!selectedBranchId ? (
        ...
        <>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Monthly branch target — set by {canSetAnyBranch ? 'Super Admin / MD / HOC' : 'Branch Manager'}</CardDescription>
                </div>
                {(canSetAnyBranch || isBM) && !editing && (
                  <Button size="sm" variant="outline" onClick={() => setEditing(true)}>
                    <Edit3 className="h-3.5 w-3.5 mr-1" /> {data.target.disbursementTarget ? 'Edit' : 'Set'} Target
                  </Button>
                )}
              </div>
            </CardHeader>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <div className="flex items-baseline justify-between mb-1">
                      ...
                      <Badge variant="outline" className="text-[10px]">{data.target.month}</Badge>
                    </div>
                    <p className="text-xl font-bold text-slate-900 mb-1">
                      ₦{(data.target.disbursementTarget || 0).toLocaleString()}
                    </p>
                    <div className="flex justify-between text-xs text-slate-500 mb-1">
                      <span>Actual: ₦{(data.actual.totalDisbursed || 0).toLocaleString()}</span>
                      <span className={cn('font-semibold', data.progress.disbursementPct >= 100 ? 'text-emerald-600' : 'text-slate-700')}>
                        {data.progress.disbursementPct}%
                      </span>
                    </div>
                    <Progress value={Math.min(data.progress.disbursementPct, 100)} className="h-2" />
                  </div>
                  <div>
                    <div className="flex items-baseline justify-between mb-1">
                      <p className="text-[11px] uppercase tracking-wider text-slate-400 font-semibold">Loan Count Target</p>
                      <Badge variant="outline" className="text-[10px]">Submitted: {data.actual.submittedLoans}</Badge>
                    </div>
                    <p className="text-xl font-bold text-slate-900 mb-1">{data.target.loanCountTarget || 0} loans</p>
                    <div className="flex justify-between text-xs text-slate-500 mb-1">
                      <span>Disbursed: {data.actual.loanCount}</span>
                      <span className={cn('font-semibold', data.progress.loanCountPct >= 100 ? 'text-emerald-600' : 'text-slate-700')}>
                        {data.progress.loanCountPct}%
                      </span>
                    </div>
                    <Progress value={Math.min(data.progress.loanCountPct, 100)} className="h-2" />
                  </div>
                </div>
              )}
            </CardContent>
          ...
          {/* LO breakdown — BM uses this to assign individual targets */}
          <Card>
            <CardHeader>
              <CardTitle className="text-sm flex items-center gap-2">
                <Trophy className="h-4 w-4 text-amber-600" /> Loan Officer Breakdown
              </CardTitle>
              <CardDescription>
                {isBM
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-700 mt-1">
                    1. Super Admin / MD / HOC sets YOUR branch target here.<br/>
